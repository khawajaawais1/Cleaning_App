...
export class ProfileComponent implements OnInit {
  ...
  serviceTypes = ['Standard Clean', 'Deep Clean', 'End of Tenancy', 'Office Clean', 'Carpet Clean'];
  ...
  ngOnInit(): void {
    this.auth.getProfile().subscribe(p => {
      this.profile = p;
      if (p) {
        this.form.patchValue({ phone: p.phone, city: p.city, serviceType: p.serviceType, bio: p.bio ?? '' });
      }
    });
  }
  ...
  get statusClass(): string {
    const map: Record<string, string> = {
      active: 'pill-green', pending: 'pill-amber', suspended: 'pill-red', rejected: 'pill-red',
    };
  ...
  toggleOnline(): void {
    ...
    this.auth.setOnline(newValue).subscribe({
      next: () => { this.togglingOnline = false; },
      error: () => {
  ...
  saveProfile(): void {
    ...
    this.http.put(`${API_BASE}/worker/me`, {
      phone: v.phone,
      city: v.city,
      serviceType: v.serviceType,
    }).subscribe({
      next: () => { this.saving = false; this.editMode = false; },
      error: () => { this.saving = false; this.editMode = false; },
    });
  }
